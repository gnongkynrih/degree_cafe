...
<div class="flex flex-wrap justify-center">
  <a href="{{ route('sale.index',[$seat->table_name]) }}" class="block max-w-[15em] p-6 m-2 rounded-md shadow hover:bg-gray-100 transition duration-300 {{ $activeLink =='all' ? 'bg-blue-500 text-white' : 'bg-white border border-gray-200 text-gray-900'}}">
    <h5 class="text-center text-2xl font-bold tracking-tight">All</h5>
  </a>
  @foreach($categories as $category)
  <a href="{{ route('sale.index', [$seat->table_name,$category->id]) }}" class="block max-w-[15em] p-6 m-2 rounded-md shadow hover:bg-gray-100 transition duration-300 {{ $activeLink == $category->id ? 'bg-blue-500 text-white' : 'bg-white border border-gray-200 text-gray-900'}}">
  ...
  <div class="md:basis-2/3 rounded-lg">
    <table class="w-full text-sm text-left text-gray-500 table-auto">
      ...
      <tbody>
        ...
        <tr id="R{{ $menu->id }}" class="even:bg-gray-100 odd:bg-white hover:bg-gray-200 transition duration-200">
          <td class="px-4 py-2">{{ $menu->name }}</td>
          <td class="px-4 py-2">{{ $menu->description }}</td>
          <td class="px-4 py-2">{{ $menu->price }}</td>
          <td class="px-4 py-2">
            <div class="flex items-center justify-center">
              <button id="M{{ $menu->id }}" class="minus text-white text-2xl  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-full mr-1 focus:outline-none transition duration-200 h-10 w-10">-</button>
              ...
              <button id="A{{ $menu->id }}" class="plus text-white text-2xl  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-full ml-1 focus:outline-none transition duration-200 h-10 w-10">+</button>
            </div>
          <lt2>
        </tr>
        @endforeach
    ...
    <table class="w-full text-sm text-left text-gray-500 table-auto">
      ...
      <tbody id="selectedItems">
        @foreach ($orders as $order)
          <tr id="S{{ $order->menu_id }}" class="even:bg-gray-100 odd:bg-white hover:bg-gray-200 transition duration-200">
